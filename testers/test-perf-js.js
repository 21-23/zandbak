...
const sandbox = zandbak({
    ...
    type: 'electron',
    options: {
        sand: 'js', // css | lodash | lodash/subworkers | js | js/subworkers
        showDevTools: false,
...
function onTaskSolved({ task, error, result, correct }) {
    console.log('[test-perf-js]', 'Task solved', task, '; error', error, '; result:', result, '; correct:', correct);
    console.log('[test-perf-js]', 'Remaining tasks', --remainingTasks);
}
...
setTimeout(() => {
    ...
    while (--counter >= 0) {
        ...
        setTimeout(((num, timeout) => {
            sandbox.exec({ id: `task-${num}`, input: `return arg.splice(1, 3).map((item) => { return item.name + item.age + '${timeout}'; })` });
